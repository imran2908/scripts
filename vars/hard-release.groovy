...
node ('INBLRCYBDTRCHP90') {
    ...
    try{
        ...
        sh '''
            	...
            	to: "212590189@gehealthcare.com,cc:212757368@gehealthcare.com",
      ...
      /*  stage('Base-Image Staging Artifactory Push') {
                docker.withRegistry('https://blr-artifactory.apps.ge-healthcare.net/docker-cyberlab-stage', '503302923_Jfrog_RT') { 
                sh '''
					        pwd
                            id
					        cd ./resources
...
//Start of Notification Script            
                    ...
                    stage ('Notify Approver') {
            	        ...
            	        to: "212590189@gehealthcare.com,cc:212757368@gehealthcare.com,cc:503302923@gehealthcare.com,cc:503335120@gehealthcare.com,cc:503357954@gehealthcare.com,cc:503314392@gehealthcare.com,cc:550005724@gehealthcare.com",
                        ...
                        <b></p>Please click <a href="${BUILD_URL}/input/">here</a> to either Approve or Deny Golden Image push to blr-artifactory.apps.ge-healthcare.net/docker-cyberlab-dev/  </p></b>
                        </table>
                        <br>
...
try {
    stage('Deploy Approval') {
                        timeout(time:180, unit:'MINUTES') {                            
                            script {
                                def userInput1 = input(id: 'userInput1', message: 'Your input is mandatory to push Golden Image to Artifactory', ok: 'Confirm Selection',  submitter: '212590189,212757368', parameters: [[$class: 'BooleanParameterDefinition', defaultValue: false, description: 'Please left-click once on the checkbox above to put a check mark and hit Confirm button below to invoke artifactory push. Otherwise hit Abort.', name: 'Push Golden Image to blr-artifactory.apps.ge-healthcare.net/docker-cyberlab-dev/']])
          ...
          stage ('Twistcli Scan') {
                ...
                if (env.Twistlock_CRITICAL == env.Expected_CRITICAL && env.Twistlock_HIGH == env.Expected_HIGH){
                    ...
                    stage('Notify Approver'){
                            ...
                            body: 
                                ...
                                </p>${BUILD_LOG_REGEX, regex=".*(Compliance found.*_harden_.*low - [0-9]*).*", linesBefore=0, linesAfter=0, maxMatches=1, showTruncatedLines=false, escapeHtml=true, matchedLineHtmlStyle=true, substText="$1"}</p><br>

                                 </p><b>================================================================================================================</b></p>  
                                    
                                <b></p>Click <a href="${BUILD_URL}/input/">${JOB_NAME}:${BUILD_NUMBER}</a> to Approve/Deny Harden Image push to Release Artifactory </p></b>
                                <br>
                                </p>If you have any questions please reach out to DTRSupport@gehealthcare.com</p>
            ...
            println(userInput3)
                            ...
                            if (userInput3 == true){
                                ...
                                to: "DTRUsers@gehealthcare.com,devsecopsdtr@ge.com,cc:DTRSupport@gehealthcare.com,DevSecOpsDTRCoreDev@ge.com",
                                ...
                                '''
                               	...
                               	<br><p><b>Get yourself added to DTR DL:</b> https://ge-hc.atlassian.net/wiki/spaces/CZJTA/pages/110235083/GEHC+Docker+Trust+Registry+DTR#6.-Email-notifications-and-links</p><br>
                                    ...
                                    <br></p>If you have any questions please reach out to DTRSupport@gehealthcare.com or DevSecOpsDTRCoreDev@ge.com</p><br>
                            ...
                            catchError(buildResult: 'Success', stageResult: 'ABORTED', message : 'Job has been timeout') 
                                ...
                                echo "COMMENTS: ${env.COMMENTS}"
                                docker.withRegistry('https://blr-artifactory.apps.ge-healthcare.net/docker-cyberlab-dev', '503302923_Jfrog_RT') {
                                        sh 'docker tag "env.HardenImage" blr-artifactory.apps.ge-healthcare.net/docker-cyberlab-dev/"env.HardenImage"_ReleaseApprovalDenied'
                                        sh 'docker push blr-artifactory.apps.ge-healthcare.net/docker-cyberlab-dev/"env.HardenImage"_ReleaseApprovalDenied'
                                        sh 'docker rmi -f blr-artifactory.apps.ge-healthcare.net/docker-cyberlab-dev/"env.HardenImage"_ReleaseApprovalDenied'
                                }
                                emailext attachLog: true,attachmentsPattern:'Release_TwistlockOutput3.*',
                                to: "212590189@gehealthcare.com,212757368@gehealthcare.com",
                                ...
                                body: 
                                    ...
                                    <br>
                                    </p>If you have any questions please reach out to DTRSupport@gehealthcare.com or DevSecOpsDTRCoreDev@ge.com</p>
                    ...
                    docker.withRegistry('https://blr-artifactory.apps.ge-healthcare.net/docker-cyberlab-dev', '503302923_Jfrog_RT') {
                                    sh 'docker tag "env.HardenImage" blr-artifactory.apps.ge-healthcare.net/docker-cyberlab-dev/"env.HardenImage"_ReleaseTwistlockFailed'
                                    sh 'docker push blr-artifactory.apps.ge-healthcare.net/docker-cyberlab-dev/"env.HardenImage"_ReleaseTwistlockFailed'
                                    sh 'docker rmi -f blr-artifactory.apps.ge-healthcare.net/docker-cyberlab-dev/"env.HardenImage"_ReleaseTwistlockFailed'
                                }
                    emailext attachLog: true,
                            attachmentsPattern:'Release_TwistlockOutput3.*',
