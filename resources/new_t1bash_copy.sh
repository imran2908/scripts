...
#echo "DOCKER_CONTENT_TRUST=1 | sudo tee -a /etc/environment" >> $file
echo "USER 0" >> $file
echo "WORKDIR /tmp" >> $file
#echo "ADD new_healthcheck.sh ./" >> $file
#echo "HEALTHCHECK CMD /tmp/new_healthcheck.sh || exit 1" >> $file
echo "COPY new_cis_debian_alpine_copy.sh ./" >> $file
echo "RUN --mount=type=secret,id=rt_psw \
        chmod a+wx new_cis_debian_alpine_copy.sh     && \
        ./new_cis_debian_alpine_copy.sh                         && \
        rm -rf ./*                                              && \
...
docker images -q | grep -oP '(?<='$hard_img').*' | xargs -r docker rmi -f <<< $hard_img &> /dev/null
