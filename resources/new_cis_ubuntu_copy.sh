...
sudo --help &> /dev/null
        if [[ $? != 0 ]]; then
                apt-get update &> /dev/null; apt-get install -y sudo --no-install-recommends &> /dev/null
        fi
...
&&      sed -i -r '/^('"$GROUP"'|root|rabbitmq|_apt|postmaster|postgres|sshd|rabbitmq|kong|nginx|nobody|operator|guest|bin|ping|wheel|man|adm|daemon|ftp|mail|sudo|kibana|irc|sys|gnats|proxy|list|www-data|users|systemd-network|dbus|lp|tty|systemd-journal|node|smmsp|ntp|vpopmail|xfs|squid|uucp|shadow|nofiles|nogroup|xfs|kvm|elasticsearch|flink|staff)/!d' /etc/group
...
find $sysdirs -xdev \( -name hexdump -o -name chgrp -o -name od -o -name mn=1 \)  -exec rm -fr {} + || \
        find $sysdirs1 -xdev \( -name hexdump -o -name chgrp -o -name od -o -name mn=1 \) -exec rm -fr {} +
