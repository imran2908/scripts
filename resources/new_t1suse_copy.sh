...
#echo "ADD new_cis_suse_copy.sh secret.txt ./" >> $file
echo "ADD new_cis_suse_copy.sh ./" >> $file
echo "RUN \
        --mount=type=secret,id=rt_psw chmod a+wx new_cis_suse_copy.sh      && \     
        ./new_cis_suse_copy.sh                          && \
        rm -rf ./*                                      && \
