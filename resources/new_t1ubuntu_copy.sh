...
echo "COPY new_cis_ubuntu_copy.sh ./" >> $file
echo "RUN --mount=type=secret,id=rt_psw \
        chmod a+wx new_cis_ubuntu_copy.sh    && \
        ./new_cis_ubuntu_copy.sh                        && \
        rm -rf ./*                                      && \
...
#docker build --no-cache --pull --build-arg http_proxy=http://cis-india-pitc-bangalore.corporate.ge.com:80 -t "$hard_img" .
        if [ $? == 0 ]; then
                echo "==================================================================================================================="
